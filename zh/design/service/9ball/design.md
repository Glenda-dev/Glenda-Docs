...
## 1. 简介
9Ball 是 Glenda 操作系统的 Root Task（相当于 PID 1）。它是内核启动的第一个用户空间程序。其主要职责是引导用户空间环境，特别是初始化 `Warren` 服务管理器，然后协调其他基本系统服务的启动。

## 2. 职责

*   **系统引导**: 初始化全局 Capability 空间和内核移交的基本资源。
*   **Warren 初始化**: 加载并启动 `Warren` 服务。由于 `Warren` 是任务管理器，9Ball 必须在将进程创建委托给 Warren 之前，手动（使用 `libglenda` 辅助函数）执行 Warren 的初始加载。
*   **服务编排**: 一旦 Warren 运行起来，9Ball 就充当系统配置管理器，根据配置指示 Warren 启动其他核心服务（例如 Unicorn, Gopher, Rio）。
...
### 第二阶段：启动 Warren
1.  9Ball 定位 `Warren` 二进制文件（通常由引导加载程序作为引导模块提供）。
2.  9Ball 为 Warren 创建一个新的保护域 (CSpace 和 VSpace)。
3.  **资源委派**: 9Ball 将大部分系统资源（未类型化内存、IRQ 控制权）转移给 Warren。这使得 Warren 能够履行其作为系统其余部分的内存和任务管理器的角色。
4.  9Ball 启动 Warren 线程。

### 第三阶段：启动系统服务
一旦 Warren 处于活动状态，9Ball 就会与其建立 IPC 通道。然后，9Ball 通过向 Warren 发送请求来启动其他服务。

**启动服务（例如 Gopher）的工作流程：**
1.  9Ball 读取 initrd 中的服务二进制文件。
2.  9Ball 向 Warren 发送 `CreateProcess` IPC 消息，提供二进制数据或位置。
3.  Warren 解析 ELF，分配内存并创建线程。
4.  Warren 将进程句柄返回给 9Ball。
5.  9Ball 向新进程发送 `Start` 消息。

### 第四阶段：监督循环
启动序列完成后，9Ball 进入阻塞循环，等待 IPC 消息或故障通知。
*   **故障处理**: 如果关键服务崩溃，Warren（作为异常处理程序）可能会通知 9Ball。
*   **系统电源**: 9Ball 处理关机或重启请求。

## 4. 接口

### 与 Warren 的交互
9Ball 作为 Warren 的客户端进行进程管理。
...
2.  **服务输出捕获**: 当 9Ball 指示 Warren 启动服务时，它会提供一个 IPC 端点 Capability 或共享内存环形缓冲区，用作该服务的 `stdout`/`stderr`。
