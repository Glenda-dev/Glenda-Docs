...
## 1. 简介
**Rc** 是 Glenda 的默认命令行 Shell。它是 Plan 9 `rc` shell 的移植/重新实现，适应了 Glenda 的微内核架构。它是与 Warren（进程控制）和 Gopher（文件操作）交互的主要用户界面。
...
*   `bind`: 绑定目录。
*   `unmount`: 卸载。
*   `rfork`: 创建新命名空间（调用带有 `RFNOMNT` 或类似系统调用的 `Warren::SPAWN`）。
...
*   **Gopher**:
    ...
    *   `MOUNT/BIND`: 命名空间操作。
*   **Tux**:
    *   Rc 本身是一个类 POSIX 应用程序，可能链接到 `libglenda` 或与 Tux/Warren 对话的 libc。
