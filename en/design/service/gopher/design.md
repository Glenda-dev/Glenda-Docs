...
## 1. Introduction
Gopher is the **Network Stack Provider** for Glenda. It implements the TCP/IP protocol stack (based on LWIP or similar) and manages network interfaces. It exposes the network stack via the **Network Protocol** (serving the `/net` hierarchy) and provides high-performance packet I/O via **Dedicated IPC**.
...
For high-bandwidth data applications, generic IPC transport overhead is too high.
...
### 3.2 Control Plane
All configuration and connection setup happens via network control protocol.
